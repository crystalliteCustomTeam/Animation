        ...
        <div className="container">
          <div className="flex items-center justify-center">
            <div className="w-full">
              <h2 className="text-[40px] md:text-[50px] font-semibold font-sans text-center leading-tight mb-3">
                {title}
              </h2>
              ...
              <div className={`${styles.cardsMainDiv}`}>
                {packages.map((pkg, index) => (
                  ...
                  >
                    <div
                      key={index}
                      className={`absolute top-[-7%] md:top-[-10%] lg:top-[-7%] xl:top-[-8%] left-[12%] h-[115px] w-[75%] -z-10 rounded-3xl flex items-start justify-center pt-4 gap-2 
                                            ${
                                              index === 1 || index === 3
                                                ? "bg-[#262626]"
                                                : "bg-third"
                    ...
                    <div
                      className={`relative ${
                        pkg.backgroundColor === "#262626"
                          ? "bg-second"
                          : "backdrop-blur-sm bg-[#2f2f2fd1]/75"
                      } flex flex-col justify-start py-12 h-auto rounded-[35px]`}
                    >
                      <div className="border-b-2 px-6 pb-4">
                        <h3 className="text-[30px] font-extrabold font-sans leading-tight mb-2">
                          {pkg.name}
                        </h3>
                        <h4
                          className={`text-[60px] font-extrabold font-sans ${
                            pkg.backgroundColor === "#2f2f2fd1"
                              ? "text-secondary-100"
                              : ""
                          }`}
                        >
                      ...
                      <ul className="pt-4 px-6 max-h-[270px] ">
                        {pkg.features.map((feature, i) =>
                          typeof feature === "string" ? (
                            <li
                              key={i}
                              className="flex items-center gap-3 my-2 xl:my-3 text-[16px] font-normal font-sans leading-snug tracking-wider"
                            >
                              <Image
                                src={pkg.tickImage}
                                className="w-[3%]"
                            ...
                            feature.isVisible && (
                              <li
                                key={i}
                                className="flex items-center gap-3 my-2 xl:my-3 text-[16px] font-normal font-sans leading-snug tracking-wider invisible"
                              >
                                <Image
                                  src={pkg.tickImage}
                                  className="w-[3%]"
                      ...
                      <div className="flex-wrap flex items-center justify-between mt-[10px] w-[90%] mx-auto">
                        <button
                          onClick={popupHandle}
                          className="flex items-center justify-center text-[11px] xl:text-[15px] text-black font-[700] poppins bg-white py-[12px] px-[10px] lg:px-[15px] tracking-[.3px] rounded-[5px] leading-[20px] shadow-2xl w-full"
                        >
                          ...
                          <React.Fragment key={i}>
                            {button.text === "1-833-666-6689" ? (
                              <a
                                href={button.href}
                                key={i}
                                className={`flex items-center justify-between font-sans ${button.color} w-max px-3 xl:px-5 gap-2 rounded-lg py-[7px]`}
                              >
                                <span className="text-[16px] xl:text-[18px] font-normal font-sans">
                                  {button.text}
                                </span>
                                <Image
                                  className={`${button.imgColor}`}
                                  src={button.image}
                              ...
                              <a
                                key={i}
                                href={button.href}
                                className={`flex items-center justify-between font-sans ${button.color} w-max py-2 px-3 xl:px-5 gap-2 rounded-lg text-[14px] xl:text-[16px]`}
                              >
                                {button.text}
                                <Image
                                  src={button.image}
                                  className={`${button.imgColor} w-[30px]`}
                                  alt="Infinity Animation"
                                />
                              </a>
                            )}
