...
const Portfolio = () => {
    ...
    return (
        <>
            <section className={`w-full flex items-center justify-start pb-[50px] sm:pt-[50px] lg:pt-[60px] lg:pb-[90px] bg-white`}>
                <div className="px-4 sm:px-8 lg:max-w-7xl mx-auto">
                    ...
                    <div className={`"mt-8 lg:mt-12`}>
                        <ul className="flex flex-wrap justify-center xl:justify-between text-sm font-medium text-center gap-2 lg:gap-5 mx-auto bg-[#DCDCDC] rounded-[10px]">
                            {tabInfo.map((tab, index) => (
                                <li key={index}
                                    className={`w-max px-[10px] lg:px-[35px] h-[35px] lg:h-[70px] flex items-center justify-center text-[12px] lg:text-[18px] rounded-[10px] font-semibold poppins cursor-pointer hover:bg-[#ff2d4b] hover:text-white hover:duration-700 duration-700 ease-in-out ${activeTab === index ? 'bg-[#FF2D4B] text-white' : 'bg-transparent text-black'}`}
                                ...
                                <Fancybox options={{
                                    ...
                                    <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3 md:gap-5'>
                                        {tabContents[activeTab].map((item, index) => (
                                            <Link key={index} href={item.video} datafancybox="gallery" className={`w-full h-full group overflow-hidden`}>
                                                <div className="overlay relative">
                                                    <Image src={item.thumbnail} alt="Infinity Animations" width={468} height={263} className='min-h-[195px] xl:min-h-[230px] rounded-[15px]' />
                                                    <div className="bg-black/70 h-full w-full absolute left-[50%] top-[59%] xl:top-[50%] translate-x-[-50%] group-hover:translate-y-[-115px] translate-y-[132px] flex items-center justify-center rounded-[15px]">
                                                        <Image src={PlayIcon} alt='Play-icon' width={80} height={80} />
                                                    </div>
                                                </div>
                                            </Link>
                                        ))}
